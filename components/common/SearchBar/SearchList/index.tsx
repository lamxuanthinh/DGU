...
import { dataUserSearch } from "./constant";
        ...
        <>
            {isFilter && (
                <div className="px-4 flex items-center mb-1 mt-[10px]">
                    <span className="font-semibold text-[15px] text-[#8A8A8A]">User</span>
            ...
            {!isCollapse && (
                <>
                    {dataUserSearch.map((item: any, index: any) => {
                        return (
                            <SearchItem
                                key={index}
                                srcImage={require(`@/public/Images/Profile/Mycourse/${item.avatarUrl}`)}
                                isUser={isUser}
                                dataUserSearch={item}
                            />
                        );
