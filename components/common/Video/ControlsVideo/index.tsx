            ...
            {Array.isArray(dataVideo) ? (
              dataVideo.map((item: IVideoShortPayload) => {
                ...
                const breakEndPointToPercent =
                  ((item.break_point + item.duration) / totalTime) * 100;

                const percentItemDuration =
                  breakEndPointToPercent - breakPointToPercent;
                const CalcPercentWidthOfControlItemToColor = () => {
                  if (breakEndPointToPercent < currentTimeByPercent) {
                    ...
                    return (
                      ((currentTimeByPercent - breakPointToPercent) /
                        percentItemDuration) *
                      100
                    );
                  }
