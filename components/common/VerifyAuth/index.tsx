...
function VerifyAuth({ heading, srcImage, description, isVerifySuccess, isVerifyError }: IVerifyAuthProps) {
     return (
          <main className="flex items-center justify-center p-3 min-h-screen">
               <Button href="/register" className="fixed top-2 left-2 text-2xl px-4 py-3">
                    <BiArrowBack />
               </Button>
               <div className="flex flex-col items-center justify-center gap-y-5">
                    <Image className="max-w-[340px]" src={srcImage} alt="image notification" priority={true} />
                    <h1 className={`font-bold text-3xl lg:text-6xl ${isVerifyError ? "text-[#FF0000]" : ""}`}>{heading}</h1>
                    <p className={`max-w-[700px] text-center text-base text-[#565656] lg:text-2xl ${isVerifyError ? "text-[#FF0000]" : ""}`}>
                         {description}
                    </p>
